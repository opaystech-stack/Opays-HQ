    ...
    <div className="relative min-h-full px-6 py-8 text-slate-900 lg:px-8 bg-[#f8f9fb]">
      ...
      <div className="relative space-y-8">
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div 
            onClick={() => canEdit && setIsTxOpen(true)}
            className={`rounded-[2.5rem] border border-slate-200 bg-white p-8 shadow-sm md:col-span-1 transition-all ${canEdit ? 'cursor-pointer hover:border-emerald-400 hover:shadow-xl' : ''}`}
          >
            ...
            {canEdit && <p className="mt-6 text-[10px] font-black uppercase tracking-widest text-emerald-600 animate-pulse">Cliquer pour ajuster</p>}
          </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {partnerships.map((partner) => (
                    <div 
                      key={partner.id} 
                      onClick={() => canEdit && alert('Édition du partenaire : ' + partner.name)}
                      className={`flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3.5 transition-all ${canEdit ? 'cursor-pointer hover:bg-white hover:border-cyan-200' : ''}`}
                    >
                      <span className="text-sm font-bold text-slate-900 uppercase tracking-tight">{partner.name}</span>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {logs.slice(0, 5).map((log) => (
                    <div 
                      key={log.id} 
                      onClick={() => canEdit && alert('Détail transaction : ' + log.description)}
                      className={`group flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 transition-all ${canEdit ? 'cursor-pointer hover:bg-white hover:border-slate-200' : ''}`}
                    >
                      <div className="min-w-0 pr-4">
                        <p className="truncate text-xs text-slate-900 font-bold uppercase tracking-tight">{log.description}</p>
